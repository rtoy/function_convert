...
\begin{enumerate}[label=(\alph*), noitemsep, topsep=0pt]
  ...
  \item The function \texttt{xgather-args-of} scans \ttq{e}. for
...
\paragraph*{Colophon} This manual was typeset using the \texttt{pdflatex} engine, the
...
All examples were executed using Maxima from
the from the current development branch together with the \funconvert package also from the current development branch.
...
For the API something like:

\appendix
\section{API Reference}

This appendix summarizes the public functions provided by the
\funconvert package. Each entry lists the function name, its arguments,
a brief description of its behavior, and any important notes or
restrictions.

\subsection{Converter Management}

\begin{description}

  

  \item[\texttt{delete\_converter(source = target)}]
    Removes a user-defined converter. Built-in converters cannot be
    removed; attempting to do so signals an error.
