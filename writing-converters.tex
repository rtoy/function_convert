...
\end{itemize}
This guide is written for people who want to create new converters. It focuses on writing converters. Here we include only the API elements that matter when you are authoring a converter.
...
\end{verbatim}
\end{maximabox} 
The first argument \ttq{erfc = erf} is called a \emph{conversion rule} (or simply a `rule'), the left-hand side is the \emph{source function} and the right-hand side \ttq{erf} is called  the
...
This use of  to define a rule is similar to its use in the Maxima function substitute, with one difference: here the operator \ttq{=} indicates a semantic conversion, not a literal renaming. One might argue that it would be clearer to define a rule using an arrow‑like operator such as \ttq{=>}. Using \ttq{=} instead of an arrow‑like symbol is consistent with substitute and preserves arrow‑like operators for other purposes.
...
Permission is granted to copy and distribute this document, in print or electronically, for any purpose, provided that the content is not modified and this notice is preserved.
\end{flushleft}
