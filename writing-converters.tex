...
name this rule \ttq{gamma = sin}, for example. Actually, this converter  built-in to the \funconvert package:
...
Docstring:
  ...
  \end{verbatim}
\end{maximabox}
If a user-package properly includes a docstring for a converter, that documentation will
...
a typo or some confusion between Maxima nouns and verbs. If a converter doesn't trigger 
...
\end{verbatim}
\end{maximabox}
\noindent implements the identity $\erfc(x) = 1 - \erf(x)$.  Notice that since \ttq{x} is a Common Lisp list
of the argument to \ttq{\%erfc}, it is necessary to apply \ttq{\%erf} to the car of \ttq{x}, and
...
%%%%%%%%%%%%%%
\section{Regression Tests for \funconvert}
...
\paragraph{Running the Tests} If your tests are in a file \ttq{rtest\_my\_function\_convert}, to run the regression tests, use:
\begin{maximabox}
\begin{verbatim}
