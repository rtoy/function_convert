...
Specifically, \ttq{function-convert} performs a \emph{preorder}
(root--first) traversal of the expression tree: each node is visited
before its children.  However, a converter is not applied at the
moment the node is visited.  Instead, the converter is applied only
\emph{after} all arguments have been recursively converted. 
...
it is necessary to quote the source function for this converter.
