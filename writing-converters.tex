...
Changing multiple conversions is handled by the function \texttt{function\_convert}; a writer of
...
This manual was typeset using the \texttt{pdflatex} engine, the
\texttt{extarticle} document class.

All examples were executed using Maxima~\cite{maxima} from
the from the current development branch together with the \funconvert package~\cite{willis-function-convert} also from the current development branch.

The body text is set in a serif typeface chosen for
clarity in extended technical prose. Code fragments and Maxima sessions
are rendered in a monospaced font with consistent verbatim handling.


Portions of this document were adapted from the project's
\texttt{README.md} on GitHub, with the permission of the author.
Bibliographic data were managed using \texttt{bibtex}.
