...
In these examples, the left-hand side of the conversion rule \ttq{erfc = erf},
is the \emph{source function} and the right-hand side is called \ttq{erf} the
...
\end{equation}
Here the left-hand-side of this identity is a product, so the source function for this identity must be multiplication (internally \ttq{mtimes}), not the gamma function. But to a user the conversion rule \texttt{”*” = sin} may seem unnatural.  The alias system allows the converter for this identity to have a much more natural name. The converter for identity Eq. (\ref{gammaid}) is built-in to the \funconvert package;  the alias for this converter is \ttq{gamma = sin}.
